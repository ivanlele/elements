...
class GetBlocksActivityTest(BitcoinTestFramework):
    ...
    def test_activity_in_block(self, node, wallet):
        ...
        for k, v in {
                ...
                'txid': txid,
                'type': 'receive',
                'vout': 2,  # ELEMENTS: vout 0=change, 1=fee, 2=target (Bitcoin has no fee output so it's 1)
        }.items():
    ...
    def test_no_address(self, node, wallet):
        ...
        assert a1['amount'] == Decimal(no_addr_tx["fee"]) / COIN + Decimal(no_addr_tx["tx"].vout[0].nValue.getAmount()) / COIN

        assert a2['type'] == "receive"
        assert a2['blockhash'] == blockhash
        # sPK lacks address.
        assert_equal(list(a2['output_spk'].keys()), ['asm', 'desc', 'hex', 'type'])
        assert a2['amount'] == Decimal(no_addr_tx["tx"].vout[0].nValue.getAmount()) / COIN
