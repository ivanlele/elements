...
class EphemeralDustTest(BitcoinTestFramework):
    ...
    def add_output_to_create_multi_result(self, result, output_value=1):  # ELEMENTS: 0-value spendable outputs are invalid
        """ Add output without changing absolute tx fee
        """
        assert len(result["tx"].vout) > 0
        assert result["tx"].vout[0].nValue.getAmount() >= output_value
        result["tx"].vout.append(CTxOut(output_value, result["tx"].vout[0].scriptPubKey))
        # Take value from first output
        result["tx"].vout[0].nValue.setToAmount(result["tx"].vout[0].nValue.getAmount() - output_value)
        result["new_utxos"][0]["value"] = Decimal(result["tx"].vout[0].nValue.getAmount()) / COIN
    ...
    def create_ephemeral_dust_package(self, *, tx_version, dust_tx_fee=0, dust_value=1, num_dust_outputs=1, extra_sponsors=None):
    ...
    def test_non_truc(self):
        ...
        res = self.nodes[0].submitpackage([dusty_tx["hex"], sweep_tx["hex"]])
        assert_equal(res["package_msg"], "transaction failed")
        # ELEMENTS: tx vsize is larger due to nAsset/nNonce overhead; compute dynamically
        assert_equal(res["tx-results"][dusty_tx["wtxid"]]["error"], f"min relay fee not met, 0 < {dusty_tx['tx'].get_vsize()}")
    ...
    def test_unspent_ephemeral(self):
        ...
        assert_equal(self.nodes[0].getrawmempool(), [])
        dusty_tx, sweep_tx = self.create_ephemeral_dust_package(tx_version=3, dust_value=10)
        ...
        dusty_tx, _ = self.create_ephemeral_dust_package(tx_version=3, dust_value=10)
