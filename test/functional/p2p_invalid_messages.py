...
class InvalidMessagesTest(BitcoinTestFramework):
    ...
    def test_size(self):
        ...
        conn = self.nodes[0].add_p2p_connection(P2PDataStore())
        error_msg = (
            ['V2 transport error: packet too large (16000014 bytes)'] if self.options.v2transport
            else ['Header error: Size too large (badmsg, 16000001 bytes)']
        )
