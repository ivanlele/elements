...
class ConfArgsTest(BitcoinTestFramework):
    ...
    def test_dir_config(self):
        ...
        conf_path = self.nodes[0].datadir_path / 'elements.conf'
    ...
    def test_config_file_parser(self):
        ...
        with open(self.nodes[0].datadir_path / 'elements.conf', 'a', encoding='utf-8') as conf:
        ...
        self.nodes[0].assert_start_raises_init_error(extra_args=[f"-conf={main_conf_file_path}", "-allowignoredconf"], expected_msg='Error: acceptnonstdtxn is not currently supported for liquidv1 chain')
    ...
    def test_config_file_log(self):
        ...
        with self.nodes[0].assert_debug_log(expected_msgs=[f"Config file: {conf_path}"]):
            self.start_node(0, ["-allowignoredconf", "-validatepegin=0", "-chain=elementsregtest"], env=env) # ELEMENTS
            self.stop_node(0)
    ...
    def test_args_log(self):
        ...
        with self.nodes[0].assert_debug_log(
                expected_msgs=[
                    ...
                    # ELEMENTS: runs as custom chain="elementsregtest"
                    f'Config file arg: chain="{self.chain}"',
    ...
    def test_seed_peers(self):
        ...
        # -fixedseeds in tests
        util.assert_equal(self.nodes[0].getblockchaininfo()['chain'],'elementsregtest')
    ...
    def test_ignored_conf(self):
        ...
        node.assert_start_raises_init_error([f"-conf={temp_conf.name}"], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "elements.conf" file which is ignored, because a '
        ...
        # error.
        self.start_node(0, [f'-conf={node.datadir_path}/elements.conf'])
        self.stop_node(0)
    ...
    def test_ignored_default_conf(self):
        ...
        node = self.nodes[0]
        (default_datadir / "elements.conf").write_text(f"datadir={node.datadir_path}\n")
        ...
        node.assert_start_raises_init_error([], re.escape(
            f'Error: Data directory "{node.datadir_path}" contains a "elements.conf" file which is ignored, because a '
            f'different configuration file "{default_datadir}/elements.conf" from data directory "{default_datadir}" '
    ...
    def test_acceptstalefeeestimates_arg_support(self):
        ...
        conf_file = self.nodes[0].datadir_path / "elements.conf"
        for chain, chain_name in {("main", "main"), ("test", "test"), ("signet", "signet"), ("testnet4", "testnet4")}:
            util.write_config(conf_file, n=0, chain=chain_name, extra_config='acceptstalefeeestimates=1\n')
            self.nodes[0].assert_start_raises_init_error(expected_msg=f'Error: acceptstalefeeestimates is not supported on {chain} chain.')
        util.write_config(conf_file, n=0, chain="elementsregtest")  # Reset to regtest
    ...
    def run_test(self):
        ...
        conf_file = default_data_dir / "elements.conf"
