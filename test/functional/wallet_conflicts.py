...
class TxConflicts(BitcoinTestFramework):
    ...
    def test_block_conflicts(self):
        ...
        tx_AB_parent = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_AB_parent, [{self.nodes[0].getnewaddress(): Decimal("19.99998")}, {"fee": Decimal("0.00002000")}]))
        ...
        tx_A_1 = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_A_1, [{self.nodes[0].getnewaddress(): Decimal("9.99998")}, {"fee": Decimal("0.00002000")}]))
        tx_B_1 = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_B_1, [{self.nodes[0].getnewaddress(): Decimal("9.99998")}, {"fee": Decimal("0.00002000")}]))
        ...
        tx_C_child = self.nodes[0].signrawtransactionwithwallet(self.nodes[0].createrawtransaction(inputs_tx_C_child, [{self.nodes[0].getnewaddress() : Decimal("19.99996")}, {"fee": Decimal("0.00002000")}]))
    ...
    def test_mempool_conflict(self):
        ...
        raw_tx = alice.createrawtransaction(inputs=[unspents[0], unspents[1]], outputs=[{bob.getnewaddress() : 49.9999}, {"fee": 0.0001}])
        tx1 = alice.signrawtransactionwithwallet(raw_tx)['hex']

        # tx2 spends unspent[1] and unspent[2], conflicts with tx1
        raw_tx = alice.createrawtransaction(inputs=[unspents[1], unspents[2]], outputs=[{bob.getnewaddress() : 49.99}, {"fee": 0.01}])
        tx2 = alice.signrawtransactionwithwallet(raw_tx)['hex']

        # tx3 spends unspent[2], conflicts with tx2
        raw_tx = alice.createrawtransaction(inputs=[unspents[2]], outputs=[{bob.getnewaddress() : 24.9899}, {"fee": 0.0101}])
        ...
        assert_equal(alice.getbalance()['bitcoin'], 25)
        ...
        assert_equal(alice.getbalance()['bitcoin'], 0)
    ...
    def test_mempool_and_block_conflicts(self):
        ...
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], 0)
        ...
        # Sends funds to bob
        raw_tx = alice.createrawtransaction(inputs=[unspents[0]], outputs=[{bob.getnewaddress() : 24.99999}, {"fee": 0.00001}])
        ...
        raw_tx = alice.createrawtransaction(inputs=[unspents[0], unspents[2]], outputs=[{alice.getnewaddress() : 49.999}, {"fee": 0.001}])
        tx1_conflict = alice.signrawtransactionwithwallet(raw_tx)['hex']

        # Sends funds to bob
        raw_tx = alice.createrawtransaction(inputs=[unspents[1]], outputs=[{bob.getnewaddress() : 24.9999}, {"fee": 0.0001}])
        ...
        raw_tx = alice.createrawtransaction(inputs=[unspents[1]], outputs=[{alice.getnewaddress() : 24.9999}, {"fee": 0.0001}])
        ...
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], Decimal("49.99989000"))

        # spend both of bob's unspents, child tx of tx1 and tx2
        raw_tx = bob.createrawtransaction(inputs=[bob_unspents[0], bob_unspents[1]], outputs=[{bob.getnewaddress() : 49.999}, {"fee": 0.00089}])
        ...
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], Decimal("49.99900000"))
        ...
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], Decimal("24.99990000"))
        ...
        raw_tx = alice.createrawtransaction(inputs=[unspents[2]], outputs=[{alice.getnewaddress() : 24.99}, {"fee": 0.01}])
    ...
    def test_descendants_with_mempool_conflicts(self):
        ...
        raw_tx = alice.createrawtransaction(inputs=[unspents[0]], outputs=[{bob.getnewaddress() : 24.9999}, {"fee": 0.0001}])
        tx1 = alice.signrawtransactionwithwallet(raw_tx)['hex']
        tx1_txid = alice.sendrawtransaction(tx1)
        ...
        raw_tx = bob.createrawtransaction(inputs=[bob.listunspent(minconf=0)[0]], outputs=[{carol.getnewaddress() : 24.999}, {"fee": 0.0009}])
        ...
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], 0)
        assert_equal(carol.getbalances()["mine"]["untrusted_pending"]['bitcoin'], Decimal("24.99900000"))

        # Alice spends first unspent again, conflicting with tx1
        raw_tx = alice.createrawtransaction(inputs=[unspents[0], unspents[1]], outputs=[{carol.getnewaddress() : 49.99}, {"fee": 0.01}])
        tx1_conflict = alice.signrawtransactionwithwallet(raw_tx)['hex']
        ...
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], 0)
        assert_equal(carol.getbalances()["mine"]["untrusted_pending"]['bitcoin'], Decimal("49.99000000"))
        ...
        raw_tx = alice.createrawtransaction(inputs=[unspents[1]], outputs=[{carol.getnewaddress() : 24.9895}, {"fee": 0.0105}])
        tx1_conflict_conflict = alice.signrawtransactionwithwallet(raw_tx)['hex']
        ...
        assert_equal(alice.getbalance()['bitcoin'], 0)
        assert_equal(bob.getbalances()["mine"]["untrusted_pending"]['bitcoin'], 0)
        assert_equal(carol.getbalances()["mine"]["untrusted_pending"]['bitcoin'], Decimal("24.98950000"))
