...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def test_duplicate_outputs(self):
        ...
        spk = self.nodes[0].getaddressinfo(address)["scriptPubKey"] # ELEMENTS
        tx = CTransaction()
        tx.vin = []
        tx.vout = [CTxOut(1 * COIN, bytearray.fromhex(spk))] * 2
        tx.nLockTime = 0
    ...
    def test_change_position(self):
        ...
        self.watchonly_utxo = self.create_outpoints(self.nodes[0], outputs=[{watchonly_address: self.watchonly_amount}], blind=True)[0]
    ...
    def test_no_change(self):
        ...
        inputs  = [ {'txid' : utx['txid'], 'vout' : utx['vout']}]
        outputs = [{self.nodes[0].getnewaddress(): Decimal(5.0) - self.test_no_change_fee - self.fee_tolerance - Decimal("0.00001500")}] # ELEMENTS: delta for larger transactions
        rawtx   = self.nodes[2].createrawtransaction(inputs, outputs)
        dec_tx  = self.nodes[2].decoderawtransaction(rawtx)
        assert_equal(utx['txid'], dec_tx['vin'][0]['txid'])
    ...
    def test_option_subtract_fee_from_outputs(self):
        ...
        utxo = self.create_outpoints(self.nodes[2], outputs=[{addr_info['unconfidential']: 10}])[0]
        self.sync_all()
        self.generate(self.nodes[0], 1)

        # An external input without solving data should result in an error
        raw_tx = self.nodes[2].createrawtransaction([utxo], [{addr_info['unconfidential']: 20}])
    ...
    def test_subtract_fee_with_presets(self):
        ...
        utxo = self.create_outpoints(self.nodes[0], outputs=[{addr: 10}], blind=True)[0]
...
#  in a transaction. See https://github.com/ElementsProject/elements/issues/880
    ...
    def test_external_inputs(self):
        ...
        # Error conditions
        assert_raises_rpc_error(-5, 'Pubkey "not a pubkey" must be a hex string', ext_fund.fundrawtransaction, raw_tx, solving_data={"pubkeys":["not a pubkey"]})
        assert_raises_rpc_error(-5, 'Pubkey "01234567890a0b0c0d0e0f" must have a length of either 33 or 65 bytes', ext_fund.fundrawtransaction, raw_tx, solving_data={"pubkeys":["01234567890a0b0c0d0e0f"]})
    ...
    def test_include_unsafe(self):
        ...
        inputs = []
        for i in range(0, 2):
            utxo = self.create_outpoints(self.nodes[2], outputs=[{addr: 5}], blind=True)[0]
