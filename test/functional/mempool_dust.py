...
class DustRelayFeeTest(BitcoinTestFramework):
    def set_test_params(self):
        self.num_nodes = 1
        self.extra_args = [['-permitbaremultisig', '-dustrelayfee=0.00003000']] # ELEMENTS: use the Bitcoin default dust relay fee rate
    ...
    def test_dust_output(self, node: TestNode, dust_relay_fee: Decimal,
        ...
        if dust_threshold > 0:
            ...
            tx.vout[0].nValue.setToAmount(tx.vout[0].nValue.getAmount() - dust_threshold)  # keep total output value constant
            ...
            # ELEMENTS: skip when threshold is 1, since threshold-1=0 is an invalid output value
            if dust_threshold > 1:
    ...
    def run_test(self):
        ...
        for dustfee_sat_kvb in (DUST_RELAY_TX_FEE, 0, 1, 66, 500, 1337, 12345, 21212, 333333):
            ...
            if dustfee_sat_kvb == DUST_RELAY_TX_FEE:
                ...
                # ELEMENTS: test_dustrelay() restarts node with [], losing -dustrelayfee; restore it here
