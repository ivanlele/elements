...
class PackageRBFTest(BitcoinTestFramework):
    ...
    def test_package_rbf_singleton(self):
        ...
        package_hex, package_txns = self.create_simple_package(singleton_coin, DEFAULT_FEE, Decimal(singleton_tx["fee"]) / COIN * 2)
    ...
    def test_package_rbf_additional_fees(self):
        ...
        assert_equal(f"package RBF failed: insufficient anti-DoS fees, rejecting replacement {package_txns3[1].rehash()}, not enough additional fees to relay; 0.00 < 0.000000{sum([tx.get_vsize() for tx in package_txns3]) // 10}", pkg_results3["package_msg"])
    ...
    def test_0fee_package_rbf(self):
        ...
        child2 = self.wallet.create_self_transfer(utxo_to_spend=parent2["new_utxo"], fee=Decimal(fees_package1 * 10) / COIN, version=3)
