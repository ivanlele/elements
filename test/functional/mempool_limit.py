...
class MempoolLimitTest(BitcoinTestFramework):
    ...
    def test_mid_package_eviction(self):
        ...
        cpfp_fee = (mempoolmin_feerate / 1000) * (cpfp_parent["tx"].get_vsize() + approx_child_vsize) - Decimal.from_float(cpfp_parent["fee"] / COIN) # ELEMENTS
        ...
        child = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos, fee=cpfp_satoshis) # ELEMENTS
    ...
    def test_mid_package_replacement(self):
        ...
        approx_child_vsize = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos)["tx"].get_vsize()
        cpfp_fee = (2 * mempoolmin_feerate / 1000) * (cpfp_parent["tx"].get_vsize() + approx_child_vsize) - Decimal.from_float(cpfp_parent["fee"] / COIN) # ELEMENTS
        child = self.wallet.create_self_transfer_multi(utxos_to_spend=parent_utxos, fee=int(cpfp_fee * COIN)) # ELEMENTS
    ...
    def run_test(self):
        ...
        tx_rich = miniwallet.create_self_transfer(fee=1) # ELEMENTS
        ...
        # ELEMENTS: fee is in satoshis
        package_fee_btc = Decimal(package_fee) / COIN
        assert_greater_than(worst_feerate_btcvb, package_fee_btc / package_vsize)
        assert_greater_than(mempoolmin_feerate, Decimal(tx_parent_just_below["fee"]) / COIN * 1000 / (tx_parent_just_below["tx"].get_vsize()))
        assert_greater_than(package_fee_btc / package_vsize, mempoolmin_feerate / 1000)
