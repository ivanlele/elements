...
void VerifyUseAvailableBalance(SendCoinsDialog& sendCoinsDialog, const WalletModel& walletModel)
{
    ...
    QVERIFY(send_entry->getValue().asset_amount == 0);
    ...
    auto value = send_entry->getValue();
    CAmountMap balance{{value.asset, value.asset_amount}};
    QVERIFY(balance == walletModel.getCachedBalance().balance);
    ...
    // the sum of them.
    Q_EMIT send_entry->useAvailableBalance(send_entry);
    QVERIFY(send_entry->getValue().asset_amount == sum_selected_coins);
}
...
void TestGUI(interfaces::Node& node, const std::shared_ptr<CWallet>& wallet)
{
    ...
    CompareBalance(walletModel, walletModel.wallet().getBalance()[::policyAsset], sendCoinsDialog.findChild<QLabel*>("labelBalance"));
    ...
    CompareBalance(walletModel, walletModel.wallet().getBalance()[::policyAsset], overviewPage.findChild<QLabel*>("labelBalance"));
...
void TestGUIWatchOnly(interfaces::Node& node, TestChain100Setup& test)
{
    ...
    walletModel.pollBalanceChanged();
    // Check balance in send dialog
    CompareBalance(walletModel, walletModel.wallet().getBalances().watch_only_balance[::policyAsset],
                   sendCoinsDialog.findChild<QLabel*>("labelBalance"));

    // Set change address
    std::map<CAsset, CTxDestination> destChange{{::policyAsset, GetDestinationForKey(test.coinbaseKey.GetPubKey(), OutputType::LEGACY)}};
    sendCoinsDialog.getCoinControl()->destChange = destChange;
