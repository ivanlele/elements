...
bool PeerManagerImpl::CanDirectFetch()
{
    if(!m_chainman.ActiveChain().Tip()) {
        LogDebug(BCLog::NET, "Tried to call CanDirectFetch with no currently-active chain.\n");
        return false;
    }
    ...
    if (WITH_LOCK(::cs_main, return m_chainman.m_best_header)) {
        ...
        if (too_far_ahead) {
            ...
            if ((nodestate->pindexBestKnownBlock == nullptr) ||
                ...
                LogDebug(BCLog::NET, "NOT discarding headers from peer=%d, to update its block availability. (current best header %d, active chain height %d)\n", pfrom.GetId(), m_chainman.m_best_header->nHeight, m_chainman.ActiveHeight());
            } else {
                LogDebug(BCLog::NET, "Discarding received headers and pausing header sync from peer=%d, because we are too far ahead of block sync. (%d > %d)\n", pfrom.GetId(), m_chainman.m_best_header->nHeight, m_chainman.ActiveHeight());
                                     ...
                                     const std::atomic<bool>& interruptMsgProc)
{
    ...
    if (msg_type == NetMsgType::CMPCTBLOCK)
    {
        ...
        if (pindex->nStatus & BLOCK_HAVE_DATA) {
            ...
            if (blockInFlightIt != mapBlocksInFlight.end()) {
                LogDebug(BCLog::NET, "Removed block %s from in-flight list because it was already downloaded\n", pindex->GetBlockHash().ToString());
            }
