...
static void WalletMigration(benchmark::Bench& bench)
{
    ...
    for (int j = 0; j < 400; ++j) {
        ...
        mtx.vout.emplace_back(::policyAsset, COIN, GetScriptForDestination(*Assert(wallet->GetNewDestination(OutputType::BECH32, strprintf("bench_%d", j)))));
        mtx.vout.emplace_back(::policyAsset, COIN, GetScriptForDestination(*Assert(wallet->GetNewDestination(OutputType::LEGACY, strprintf("legacy_%d", j)))));
        mtx.vout.emplace_back(::policyAsset, COIN, scripts_watch_only.at(j % NUM_WATCH_ONLY_ADDR));
        mtx.vin.resize(2);
