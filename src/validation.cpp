...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!bypass_limits && ws.m_ptx->nVersion != TRUC_VERSION && ws.m_modified_fees < m_pool.m_opts.min_relay_feerate.GetFee(package_size)) {
        // Even though this is a fee-related failure, this result is TX_MEMPOOL_POLICY, not
        // TX_RECONSIDERABLE, because it cannot be bypassed using package validation.
        return state.Invalid(TxValidationResult::TX_MEMPOOL_POLICY, "min relay fee not met",
                             strprintf("%d < %d", ws.m_modified_fees, m_pool.m_opts.min_relay_feerate.GetFee(package_size)));
    }
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out, const CTxIn& txin, const CScriptWitness& pegin_witness, const std::vector<std::pair<CScript, CScript>>& fedpegscripts)
{
    ...
    if (!txin.m_is_pegin) {
        ...
        if (!IsValidPeginWitness(pegin_witness, fedpegscripts, txin.prevout, err, false)) {
            LogError("%s: peg-in occurred without proof", __func__);
            fClean = false;
        } else {
            ...
            if (!fSpent) {
                LogError("%s: peg-in bitcoin txid not marked spent", __func__);
                fClean = false;
            } else {
...
static bool CheckWitnessMalleation(const CBlock& block, bool expect_witness_commitment, BlockValidationState& state)
{
    if (expect_witness_commitment) {
        ...
        if (commitpos != NO_WITNESS_COMMITMENT) {
            ...
            // ELEMENTS: check for vtxinwit size before accessing it
            std::vector<std::vector<unsigned char>> stack{};
            if (block.vtx[0]->witness.vtxinwit.size() > 0) {
                stack = block.vtx[0]->witness.vtxinwit[0].scriptWitness.stack;
            }
            const auto& witness_stack{stack};
 ...
 */
static bool ContextualCheckBlockHeader(const CBlockHeader& block, BlockValidationState& state, BlockManager& blockman, const ChainstateManager& chainman, CBlockIndex* pindexPrev) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    AssertLockHeld(::cs_main);
    ...
    while (coins_left > 0) {
        try {
            ...
            for (size_t i = 0; i < coins_per_txid; i++) {
                ...
                if (coin.out.nValue.IsExplicit() && !MoneyRange(coin.out.nValue.GetAmount())) { // ELEMENTS
