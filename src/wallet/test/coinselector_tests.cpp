...
BOOST_AUTO_TEST_CASE(bnb_sffo_restriction)
{
    ...
    auto result = WITH_LOCK(wallet->cs_wallet, return SelectCoins(*wallet, available_coins, /*pre_set_inputs=*/{}, CAmountMap{{::policyAsset, COIN}}, /*coin_control=*/{}, params));
...
BOOST_AUTO_TEST_CASE(coin_grinder_tests)
{
...

    {
        ...
        SelectionResult expected_result({{::policyAsset, CAmount(0)}}, SelectionAlgorithm::CG);
...
static util::Result<SelectionResult> select_coins(const CAmountMap& target, const CoinSelectionParams& cs_params, const CCoinControl& cc, std::function<CoinsResult(CWallet&)> coin_setup, const node::NodeContext& m_node)
{
    std::unique_ptr<CWallet> wallet = NewWallet(m_node);
