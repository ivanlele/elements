...
struct FuzzedWallet {
    ...
    void FundTx(FuzzedDataProvider& fuzzed_data_provider, CMutableTransaction tx)
    {
        ...
        for (size_t idx = 0; idx < tx.vout.size(); idx++) {
            ...
            CRecipient recipient = {dest, tx_out.nValue.GetAmount(), CAsset(), CPubKey(), subtract_fee_from_outputs.count(idx) == 1};
...
FUZZ_TARGET(wallet_notifications, .init = initialize_setup)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 200)
    {
            ...
            [&] {
                ...
                while (!coins.empty()) {
                    ...
                    LIMITED_WHILE(in > 0 && fuzzed_data_provider.ConsumeBool(), 10)
                    {
                        ...
                        CTxOut txout(CConfidentialAsset(::policyAsset), out_value, wallet.GetScriptPubKey(fuzzed_data_provider));
                        tx.vout.emplace_back(txout);
                    }
                    // Spend the remaining input value, if any
                    auto& wallet{fuzzed_data_provider.ConsumeBool() ? a : b};
                    CTxOut txout(CConfidentialAsset(::policyAsset), in, wallet.GetScriptPubKey(fuzzed_data_provider));
                    tx.vout.emplace_back(txout);
                    // Add tx to block
        ...
        if (!first_block.vtx.empty()) {
            ...
            const auto bal_a = GetBalance(*a.wallet).m_mine_trusted[::policyAsset];
            const auto bal_b = GetBalance(*b.wallet).m_mine_trusted[::policyAsset];
