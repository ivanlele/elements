...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    if (best_weight < std::numeric_limits<int>::max()) {
        ...
        auto result_cg = CoinGrinder(group_pos, {{::policyAsset, target}}, coin_params.m_min_change_target, high_max_weight);
        assert(result_cg);
    ...
    auto result_cg = CoinGrinder(group_pos, {{::policyAsset, target}}, coin_params.m_min_change_target, low_max_weight);
