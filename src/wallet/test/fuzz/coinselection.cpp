...
FUZZ_TARGET(coin_grinder)
{
    ...
    auto result_cg = CoinGrinder(group_pos, {{::policyAsset, target}}, coin_params.m_min_change_target, MAX_STANDARD_TX_WEIGHT);
    ...
    if (result_srd && result_srd->GetChange(CHANGE_LOWER, coin_params.m_change_fee)[::policyAsset] > 0) { // exclude any srd solutions that don’t have change, err on excluding
    ...
    if (result_knapsack && result_knapsack->GetChange(CHANGE_LOWER, coin_params.m_change_fee)[::policyAsset] > 0) { // exclude any knapsack solutions that don’t have change, err on excluding
...
FUZZ_TARGET(coin_grinder_is_optimal)
{
    ...
    if (best_weight < std::numeric_limits<int>::max()) {
        ...
        auto result_cg = CoinGrinder(group_pos, {{::policyAsset, target}}, coin_params.m_min_change_target, high_max_selection_weight);
        assert(result_cg);
        assert(result_cg->GetWeight() <= high_max_selection_weight);
        assert(result_cg->GetSelectedEffectiveValue()[::policyAsset] >= target + coin_params.m_min_change_target);
        assert(best_weight < result_cg->GetWeight() || (best_weight == result_cg->GetWeight() && best_amount <= result_cg->GetSelectedEffectiveValue()[::policyAsset]));
        if (result_cg->GetAlgoCompleted()) {
            ...
            assert(best_amount == result_cg->GetSelectedEffectiveValue()[::policyAsset]);
        }
    }
    ...
    auto result_cg = CoinGrinder(group_pos, {{::policyAsset, target}}, coin_params.m_min_change_target, low_max_selection_weight);
...
FUZZ_TARGET(coinselection)
{
    ...
    const auto dust{GetDustThreshold(CTxOut{CAsset(), /*nValueIn=*/0, change_out_script}, coin_params.m_discard_feerate)};
