 ...
 * @returns The result of this coin selection algorithm, or std::nullopt
 */
util::Result<SelectionResult> CoinGrinder(std::vector<OutputGroup>& utxo_pool, const CAmountMap& map_selection_target, CAmount change_target, int max_selection_weight)
{
    // ELEMENTS: only use CoinGrinder for single-asset selections
    assert(map_selection_target.size() == 1);
    CAmount selection_target = map_selection_target.begin()->second;
    ...
    SelectionResult result(map_selection_target, SelectionAlgorithm::CG);
    bool is_done = false;
    size_t curr_try = 0;
    while (!is_done) {
...
util::Result<SelectionResult> KnapsackSolver(std::vector<OutputGroup>& groups, const CAmount& nTargetValue,
                                              CAmount change_target, FastRandomContext& rng, int max_selection_weight, const CAsset& asset)
{
