...
static DBErrors LoadLegacyWalletRecords(CWallet* pwallet, DatabaseBatch& batch, int last_client) EXCLUSIVE_LOCKS_REQUIRED(pwallet->cs_wallet)
{
    ...
    // ELEMENTS
    // onlinekey
    LoadResult onlinekey_res = LoadRecords(pwallet, batch, "onlinekey",
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        CPubKey online_key;
        value >> online_key;
    ...
    result = std::max(result, onlinekey_res.m_result);

    // offlinexpub
    LoadResult offlinexpub_res = LoadRecords(pwallet, batch, "offlinexpub",
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        std::vector<unsigned char> vxpub;
        CExtPubKey xpub;
        value >> vxpub;
    ...
    result = std::max(result, offlinexpub_res.m_result);

    // offlinecounter
    LoadResult offlinecounter_res = LoadRecords(pwallet, batch, "offlinecounter",
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        int counter;
        value >> counter;
    ...
    result = std::max(result, offlinecounter_res.m_result);
    ...
    result = std::max(result, offlinedesc_res.m_result);
    ...
    result = std::max(result, blindingderivationkey_res.m_result);

    // specificblindingkey
    LoadResult specificblindingkey_res = LoadRecords(pwallet, batch, "specificblindingkey",
        [] (CWallet* pwallet, DataStream& key, DataStream& value, std::string& err) {
        uint160 scriptid;
        key >> scriptid;
    ...
    result = std::max(result, specificblindingkey_res.m_result);
    // END ELEMENTS
