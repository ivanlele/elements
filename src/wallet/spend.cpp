...
TxSize CalculateMaximumSignedTxSize(const CTransaction &tx, const CWallet *wallet, const CCoinControl* coin_control)
{
    ...
    return CalculateMaximumSignedTxSizeWithDummy(tx, wallet, txouts, coin_control); // ELEMENTS
}
        ...
        const IssuanceDetails* issuance_details) EXCLUSIVE_LOCKS_REQUIRED(wallet.cs_wallet)
{
    ...
    if (coin_control.destChange.size() > 0) {
        ...
        for (const COutPoint& presetInput : coin_control.ListSelected()) {
            CAsset asset;
            const auto& it = wallet.mapWallet.find(presetInput.hash);
            if (it != wallet.mapWallet.end()) {
                 asset = it->second.GetOutputAsset(wallet, presetInput.n);
            } else if (auto txout = coin_control.GetExternalOutput(presetInput); txout) {
                asset = txout.value().nAsset.GetAsset();
            } else {
    ...
    for (const auto& recipient : vecSend)
    {
        CTxOut txout(recipient.asset, recipient.nAmount, GetScriptForDestination(recipient.dest));
