...
{
    return std::any_of(tx->vout.cbegin(), tx->vout.cend(), [&](const auto& output) {
        return output.nAsset.IsExplicit() && output.nAsset.GetAsset() == policyAsset && IsDust(output, dust_relay_rate);
    });
}
        ...
        for (const auto& tx_input : tx->vin) {
            ...
            if (parent_ref) {
                for (uint32_t out_index = 0; out_index < parent_ref->vout.size(); out_index++) {
                    ...
                    // ELEMENTS: only treat policy asset outputs as ephemeral dust
                    if (tx_output.nAsset.IsExplicit() && tx_output.nAsset.GetAsset() == policyAsset && IsDust(tx_output, dust_relay_rate)) {
