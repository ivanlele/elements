...
BOOST_FIXTURE_TEST_CASE(package_sanitization_tests, TestChain100Setup)
{
    ...
    tx_zero_1.vout.emplace_back(CAsset(), CENT, P2WSH_OP_TRUE);
    tx_zero_2.vout.emplace_back(CAsset(), 2 * CENT, P2WSH_OP_TRUE);
...
BOOST_FIXTURE_TEST_CASE(noncontextual_package_tests, TestChain100Setup)
{
    ...
    // 24 Parents and 1 Child
    {
        ...
        child.vout.emplace_back(::policyAsset, 47 * COIN, spk2);
    ...
    // 2 Parents and 1 Child where one parent depends on the other.
    {
        ...
        mtx_parent.vout.emplace_back(::policyAsset, 20 * COIN, spk);
        mtx_parent.vout.emplace_back(::policyAsset, 20 * COIN, spk2);
        CTransactionRef tx_parent = MakeTransactionRef(mtx_parent);

        CMutableTransaction mtx_parent_also_child;
        mtx_parent_also_child.vin.emplace_back(COutPoint(tx_parent->GetHash(), 0));
        mtx_parent_also_child.vout.emplace_back(::policyAsset, 20 * COIN, spk);
        ...
        mtx_child.vout.emplace_back(::policyAsset, 39 * COIN, spk);
        CTransactionRef tx_child = MakeTransactionRef(mtx_child);
...
BOOST_FIXTURE_TEST_CASE(package_witness_swap_tests, TestChain100Setup)
{
    ...
    mtx_mixed_child.vout.emplace_back(CAsset(), (48 + 49 + 50 - 1) * COIN, mixed_child_spk);
