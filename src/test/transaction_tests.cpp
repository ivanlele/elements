...
BOOST_AUTO_TEST_CASE(tx_oversized)
{
    auto createTransaction =[](size_t payloadSize) {
        ...
        tx.vin.resize(1);
        tx.vout.emplace_back(CAsset(), 1, CScript() << OP_RETURN << std::vector<unsigned char>(payloadSize));
...
BOOST_AUTO_TEST_CASE(test_big_witness_transaction)
{
    ...
    for(uint32_t i = 0; i < mtx.vin.size(); i++) {
        ...
        vChecks.push_back(new CScriptCheck(coins[tx.vin[i].prevout.n].out, tx, signature_cache, i, SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_WITNESS, false, &txdata));
...
BOOST_AUTO_TEST_CASE(test_IsStandard)
{
    ...
    t.vout.insert(t.vout.end(), MAX_DUST_OUTPUTS_PER_TX, {t.vout[0].nAsset, CConfidentialValue(0), t.vout[0].scriptPubKey});
