    ...
    bool BatchWrite(CoinsViewCacheCursor& cursor, const uint256&) final
    {
        for (auto it{cursor.Begin()}; it != cursor.End(); it = cursor.NextAndMaybeErase(*it)) {
            if (it->second.IsDirty()) {
                if (it->second.coin.IsSpent() && (it->first.second.n % 5) != 4) {
                    m_data.erase(it->first.second);
                } else if (cursor.WillErase(*it)) {
                    m_data[it->first.second] = std::move(it->second.coin);
                } else {
                    m_data[it->first.second] = it->second.coin;
                }
            } else {
                /* For non-dirty entries being written, compare them with what we have. */
                auto it2 = m_data.find(it->first.second);
