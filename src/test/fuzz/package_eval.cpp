    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 300)
    {
        ...
        while (txs.size() < num_txs) {
            ...
            const CTransactionRef tx = [&] {
                ...
                CAmount amount_in{0};
                tx_mut.witness.vtxinwit.resize(num_in); // ELEMENTS
                for (size_t i = 0; i < num_in; ++i) {
                    ...
                    // in.scriptWitness.stack = script_wit_stack; // ELEMENTS created below

                    tx_mut.vin.push_back(in);
                    // ELEMENTS
                    tx_mut.witness.vtxinwit[i] = CTxInWitness();
                    tx_mut.witness.vtxinwit[i].scriptWitness.stack = script_wit_stack;
                }
                ...
                if (last_tx && amount_in > 1000 && fuzzed_data_provider.ConsumeBool()) {
                    tx_mut.vout.emplace_back(CAsset(), 1000, CScript() << std::vector<unsigned char>(33, 0x02) << OP_CHECKSIG);
                ...
                for (int i = 0; i < num_out; ++i) {
                    tx_mut.vout.emplace_back(CAsset(), amount_out, P2WSH_EMPTY);
                }
