...
const std::vector<std::string> RPC_COMMANDS_NOT_SAFE_FOR_FUZZING{
    ...
    // ELEMENTS
    "tweakfedpegscript",     // crashes (will fix after rebase)
};
...
std::string ConsumeScalarRPCArgument(FuzzedDataProvider& fuzzed_data_provider, bool& good_data)
{
    ...
    CallOneOf(
        ...
        [&] {
            // base64 encoded psbt
            std::optional<PartiallySignedTransaction> opt_psbt = ConsumeDeserializable<PartiallySignedTransaction>(fuzzed_data_provider, TX_WITH_WITNESS);
            if (!opt_psbt) {
                good_data = false;
                return;
            }
